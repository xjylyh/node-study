...
;(function(name,definition){
	...
	*/
    ...
    缓冲摩擦运动
    参数1：dom对象
    参数2：运动起点（top、bottom、left、right）（string）
    参数3：运动距离
    */
    
   o.animate = {
		   //dom对象，运动方式、方向，运动距离，摩擦力因数
      jump:function(obj,run,iTarget,force){
    	 switch(run){//判断起点方向
    	 case "left":
    		 var iSpeed = 0;//摩擦力因数
    	     var left = 0;//这里是最终判断是否接近目标点
    		 clearInterval(obj.timer);//防止多重定时器，先做关闭定时器
    		 obj.timer = setInterval(function(){
    		  iSpeed += (iTarget - obj.offsetLeft)/5;//加速度和摩擦力比较好的组合是5和0.7  即iSpeed+=(iTarget-obj.offsetLeft)/5 ; iSpeed*=0.7;
    		  iSpeed *= force  || 0.7;//摩擦力
    		  left += iSpeed;//left等于经过摩擦处理的加速度的总和
    		  if(Math.abs(iSpeed)<1 && Math.abs(left-iTarget)<1){//当加速度越来越小，目标点越来越近小于1的时候代表非常接近
    		   clearInterval(obj.timer);//停止定时器
    		   obj.style.left = iTarget + 'px';//对象对应值变为目标点
    		  }else{
    		   obj.style.left = obj.offsetLeft + iSpeed + 'px';//运动过程中对象的变化公式为offsetLeft加上经过摩擦处理的加速度
    		  }
    		 }, 30);//这里延时一定要短，延时过长动画不流畅
    		 break;
    	 case "right":
    		 var iSpeed = 0;
    		    var right = 0;
    	 ...
    	 case "bottom":
    		 ...
    		 clearInterval(obj.timer);
    		 obj.timer = setInterval(function(){
    		  iSpeed += (obj.offsetTop - iTarget)/5;
    		  iSpeed *= force  || 0.7;
    		  bottom += iSpeed;
    		  if(Math.abs(iSpeed)<1 && Math.abs(iTarget-bottom)<1){
    		   clearInterval(obj.timer);
    		   obj.style.top = iTarget + 'px';
    		  }else{
    		   obj.style.top = obj.offsetTop - iSpeed + 'px';
    ...
    o.cookie = {
        ...
        del:function (name, path, domain, secure) {
            o.cookie.set(name, '', -1, path, domain, secure);
        }
    };
    ...
    if (/(iPhone|iPad|iPod|iOS|android)/gim.test(navigator.userAgent)) {
        if (!(/MicroMessenger/gim.test(navigator.userAgent))) {
            ...
            if(document.querySelector(".wx_msg_bbtn")){
                document.querySelector(".wx_msg_bbtn").style.display = "none";
            }
            if(document.querySelector(".mw_new_btn")){
                document.querySelector(".mw_new_btn").style.display = "none";
            }
        }
    }
    /*//fastclick引入
